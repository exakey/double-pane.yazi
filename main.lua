...
local function _get_command(filename)
        ...
        local child, err =
            Command("bash"):args({ "-c", "cat " .. filename .. " | fzf" }):cwd(cwd):stdin(Command.INHERIT):stdout(
            Command.PIPED):stderr(Command.INHERIT):spawn()
        ...
        elseif action == "shell_fzf" then
                ...
                if filename then
                        ...
                        if not file then
                                ...
                                if cmd ~= "" then
                                        -- Parse command
                                        local _, _, run, desc, block = cmd:find(
                                        "run%s*=%s*\"(.-)\"%s*,%s*desc%s*=%s*\"(.-)\"%s*,%s*block%s*=%s*([^%s\n]+)")
